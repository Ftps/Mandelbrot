...
NUM* add_num(NUM *n1, NUM *n2, int f, bool w)
{
    NUM *add, *big, *small;
    ...
    if(n1->is_zero){add = num_cpy(n2); a = 0; LOG}
    else if(n2->is_zero){add = num_cpy(n1); a = 0; LOG}
...
NUM* mult_num(NUM *n1, NUM *n2, int f)
{
    ...
    for(int i = 0; i < _SIZE; ++i){
        if(n2->mant[_SIZE-1-i]){
            extra->exp = i;
            mult = add_num(mult, extra, 0b01, 0);
        }
    }
...
NUM* inv_num(NUM* n, bool f)
{
    ...
    for(int i = 0; i < (log2(_SIZE)+1); ++i){
        ...
        aux->sig = aux->sig^0b1;
        iter = mult_num(iter, add_num(two, aux, 0b10, 0), 0b11);
    }
